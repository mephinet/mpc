...
sub get_status {
    ...
    ok(my $status = decode_json($status_json), 'get_status return valid JSON');
...
is(get_state($mpc), 'stop', 'correct state after connect');
...
ok(my $queue = decode_json($queue_json), 'get_queue return valid JSON');
